...
import org.shenyuanv.ai.path.AStarNode;
import org.shenyuanv.ai.path.AStarSearch;
...
//		}
	...
	public int getHeight() {
		return currentAnim.getBitmap().getHeight();
	}

	public int getWidth() {
		return currentAnim.getBitmap().getWidth();
	}
	...
	public void draw(Canvas g,int offsetX,int offsetY) {
		...
		if(isSelected()){
			if(getType()!=gridMap.getTileMapRender().getCurrentType()){
				...
				RectF oval = new RectF(x+getWidth()/2, y + getHeight() / 4, getWidth()/4, getHeight() / 2);
				//g.drawArc(oval, 0, 360, true, paint);
			}else{
				paint.setColor(Color.GREEN);
				paint.setStyle(Style.STROKE);
				RectF oval = new RectF(x, y*1.07f,x+getWidth(),y+getHeight()/1.5f);
				g.drawArc(oval, 0, 360, true, paint);
				g.drawText(x+","+y, x, y, paint);
			}
			RectF rect = new RectF(x, y + getHeight()-5,  Math.round(getWidth()*(health)), 3);
			//g.drawRect(rect,paint);
			paint.setColor(Color.BLACK);
			rect = new RectF(x, y + getHeight()-5, getWidth(), 3);
			//g.drawRect(rect,paint);
			
		}
		paint = new Paint();
		//Log.d("Sprite draw", currentAnim.getBitmap().toString());
		g.drawBitmap(currentAnim.getBitmap(), x, y, paint);	
	}
	...
	public static class Animation{
		...
		public Animation(Bitmap bitmap,int endTime) {
			this.addFrame(bitmap, endTime);
		}
		...
		public void addFrame(Bitmap bitmap, int endTime) {

			totalDuration += endTime;
			frames.add(new Frame(bitmap, totalDuration));
		}
		...
		public Bitmap getBitmap() {
			return frames.get(currentIndex).Bitmap;
		}
		...
		public static class Frame {
			...
			public Frame(Bitmap bitmap, long endTime) {
				super();
				this.Bitmap = bitmap;
