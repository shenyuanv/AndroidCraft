	...
	public void draw(Canvas g,int offsetX,int offsetY) {	
		...
		if(isFighting()){			
			...
			if(targetX==tileX || targetY==tileY){
				if(tileX<targetX){
					g.drawBitmap(Constant.SCV_SPARK, x+18, y,paint);
				}else if(tileX>targetX){
					g.drawBitmap(Constant.SCV_SPARK, x-18, y,paint);
				}
				if(tileY<targetY){
					g.drawBitmap(Constant.SCV_SPARK, x, y+18,paint);
				...
				if(tileX<targetX){
					if(tileY<targetY){
						g.drawBitmap(Constant.SCV_SPARK, x+15, y+18,paint);	
					}else{
						g.drawBitmap(Constant.SCV_SPARK, x+15, y-18,paint);
					}
		...
		if(isReadyBuild()&&type==gm.getCurrentType()){
			...
			for(int y=location.y;y<location.y+size.y;++y){
				for(int x = location.x;x<location.x+size.x;++x){
					...
					if(gm.getGridMap().contains(x, y)){
						...
						g.drawRect(position.x-gm.offsetX, position.y-gm.offsetY, GridMapRender.TILE_WIDTH, GridMapRender.TILE_HEIGHT,paint);
					}else{
			...
			g.drawBitmap(icon.getTileImage(type), position.x-gm.offsetX, position.y-gm.offsetY, paint);
		}
		...
		if(isBuilding()){
			...
			int y = Math.round(this.y-offsetY);
			 //调整光照效果的位置
			 if(tileX<GridMapRender.pxToTileX(buildX)){
				 g.drawBitmap(Constant.SCV_SPARK, x+30, y,paint);
			 }else if(tileX>GridMapRender.pxToTileX(buildX)){
				 g.drawBitmap(Constant.SCV_SPARK, x-30, y,paint);
			 }
			 if(tileY<GridMapRender.pxToTileY(buildY)){
				 g.drawBitmap(Constant.SCV_SPARK, x, y+15,paint);
			 }else if(tileY>GridMapRender.pxToTileY(buildY)){
				 g.drawBitmap(Constant.SCV_SPARK, x, y-15,paint);
			 }
		}
